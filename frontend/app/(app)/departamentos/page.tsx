    ...
    <div className="space-y-4">
      ...
      {podeGerir && (
        <div className="cartao">
          ...
          <div className="flex flex-col lg:flex-row gap-2.5">
            ...
            <button className="btn-primario lg:w-auto" onClick={criar}>＋ Criar</button>
          </div>
        </div>
      )}

      <div className="cartao envolvente-tabela overflow-x-auto">
        <table className="w-full tabela-adaptavel lg:min-w-[520px]">
          <thead>
              ...
              <tr key={d.id}>
                ...
                {podeGerir && (
                  <td className="td text-right whitespace-nowrap" data-accoes>
                    <button className="btn-contorno !min-h-0 btn-mini mr-1" onClick={() => setEditar({ ...d })}>Renomear</button>
                    <button className="btn-contorno !min-h-0 btn-mini" onClick={() => eliminar(d)}>Eliminar</button>
                  </td>
                )}
              </tr>
            ))}
            {deps.length === 0 && <tr><td colSpan={4} className="td vazio">Nenhum departamento registado.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {editar && (
        <div className="modal-fundo" onClick={(e) => e.target === e.currentTarget && setEditar(null)}>
          <div className="modal-caixa lg:max-w-md">
            ...
            <div className="modal-rodape">
              ...
              <button className="btn-primario flex-1 lg:flex-none" onClick={guardarEdicao}>Guardar</button>
            </div>
          </div>
        </div>
      )}
